...
def calculate_net(gross_price, vat_percentage):
    """Calculate net price from gross price (IVA included)"""
    return gross_price / (1 + vat_percentage)

def reset_invoice():
    """Clear all invoice data and reset form fields"""
    st.session_state.invoice_items = []
    st.session_state.invoice_number = ""
    st.session_state.invoice_date = date.today()
    st.session_state.client_name = ""
...
with st.sidebar:
    ...
    # Client info fields (will be used for B2C completa and B2B)
...
with st.form("add_product", clear_on_submit=True):
    ...
    if submitted and name_input.strip() != "":
        ...
        st.success(f"✓ Added: {name_input} - €{gross_price:.2f}")
        st.rerun()
...
if st.session_state.invoice_items:
    ...
    st.divider()
    st.markdown(f"### 💰 Total client pays: €{total_gross:.2f}")
    st.caption(
        f"Net: €{total_net:.2f} | "
                    ...
                    try:
                        ...
                        checkout_session = stripe.checkout.Session.create(
                            ...
                            metadata={
                                ...
                                "vat_21": str(round(total_vat_21, 2)),
                                "vat_10": str(round(total_vat_10, 2)),
                                "created_by": username
                            }
                        )
                        ...
                        col_new, col_clear, col_copy = st.columns(3)
                        ...
                        with col_copy:
                            st.caption("💡 Select and copy the link above")
...
# =========================================================
    ...
    st.info("💡 Add your first product or service above")
    
    # Show a quick tip if invoice number is missing
    if not invoice_number:
        st.warning("📝 Don't forget to enter an invoice number in the sidebar")
